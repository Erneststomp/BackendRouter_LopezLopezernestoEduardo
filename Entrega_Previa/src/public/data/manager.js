...
export class Characters{ 
        ...
        getAll =async()=>{
            const response =  await fs.promises.readFile('./Entrega_Previa/src/public/data/data3.json','utf-8');
        ...
        SaveCharacter = async(newObject) =>{
            try {
                ...
                if(data.length === 0){
                    newObject.id=1;
                    data.push(newObject);
                    await fs.promises.writeFile('./Entrega_Previa/src/public/data/data3.json', JSON.stringify(data, null, '\t'))
                }else{
